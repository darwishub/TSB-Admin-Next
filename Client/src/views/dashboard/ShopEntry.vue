<template>
    <form>
        ...
        <PageExplanation class="no-toggle" is-open :logo="adminAuth?.photo"
                    ...
                    <div class="d-flex justify-content-start align-items-center">
                        <span class="d-inline-block">
                            <Button class="blue rounded-md p-1 ps-3 pe-3 me-3" type="button"
                                ...
                                @click="saveChanges" v-if="contentType != undefined">
                    ...
                    <div class="row justify-content-between">
                        <div class="col-5">
                            ...
                            <BaseSelect :options="programGroupList" name="program_group_id" label="Program Group"
                                class="mb-3 w-100" v-if="programGroup.length > 0" value-by="programGroupId"
                                label-by="groupName" placeholder="Select a group" style="width: 300px;"
                                @onSelect="onSelectProgramGroup" :value="programGroupId" />
                            ...
                            <BaseSelect :options="eventTypeList" name="event_type" label="Choose type of event"
                                ...
                                @onSelect="selectEventType" is-required v-if="contentType == 0" />
                            ...
                            <BaseDatePicker name="date" label="Event date" placeholder="Select date of the event"
                                is-required v-if="contentType == 0" />
                        </div>
                        <div class="col-5">
                            <BaseInput name="title" label="Title" class="mb-3"
                                placeholder="Enter the content' title here" is-required />

                            <BaseTextarea name="description" label="Description" class="mb-3"
                                placeholder="Enter the content' description here" :is-required="contentType != 2"
                                v-if="contentType != 2" />
                            ...
                            <div class="d-flex flex-column coins-amount-wrapper" v-if="contentType == 2">
                                ...
                                <div class="d-flex align-items-end justify-content-center mb-3">
                                    ...
                                    is-disabled value="1000" is-required />
                        ...
                        <div class="row">
                            <div class="col-6">
                                <BaseSelect :options="actionTypeList" name="price_type"
                                    ...
                                    value-by="value" label-by="label" @onSelect="selectActionType" class="mb-3"
                                    is-required 
                                    />

                                <BaseInput name="price"
                                    :label="actionType > 0 && actionType == 1 ? 'Price' : actionType > 0 && actionType == 2 ? 'Amount of coins reward' : ''"
                                    class="mb-3 w-100" placeholder="Enter amount of coins here"
                                    :class="{ invisible: actionType == 0 }" type="text" type-rule="coins" is-required />
        ...
        <div class="container" v-if="contentType != 2">
            <div class="shadow-out--sm radius-16 px-5 pb-5 pt-4 bg-white mb-6 text-editor">
                ...
                <div class="text-danger text-bold fs-14 mt-1" v-show="contentErrorMessage">{{ contentErrorMessage }}
                </div>
            </div>
        </div>
        <div class="container" v-if="contentType != 2">
            <div class="shadow-out--sm radius-16 px-5 pb-5 pt-4 bg-white mb-6 text-editor">
                ...
                <QuillEditor v-model:content="textDescription" theme="snow" name="description" toolbar="full"
                    placeholder="Compose detail content here..." contentType="html"
                    @update:content="onDescriptionUpdate" />
                <div class="text-danger text-bold fs-14 mt-1" v-show="descriptionErrorMessage">{{
...
const { fetchUserOnboardingData, SubmitShopEntry, SaveShopEntry } = useShopStore();
...
const saveChanges = () => {
    ...
    SaveShopEntry(formData).then((res) => {
        if (res.status == 200) {
            const item = res.data;
            return router.push(`/shop-entry-edit/${item?.name}/${item?.item_type}/${item?.id}`);
        }
    }).finally(() => {
        setTimeout(() => {
            buttonPublishStatus.disabled = false;
