    ...
    <div class="card-goal-wrapper card shadow--sm radius-16">
        <div class="card-header bg-white radius-16 shadow--md">
            <div class="row justify-content-between align-items-center">
                <div class="col-8">
                    <div class="d-flex justify-content-start align-items-center">
                        <Badge class="goal-label me-2 blue" v-for="label in labels">{{ label }} </Badge>
                    </div>
                </div>
                <div class="col-4">
                    <div class="d-flex justify-content-end align-items-center">
                        
                        <TextLevelGoal :level-number="props.item?.id_level"  
                        :class="{invisible: props.item?.id_level == 1}"
                        />
                        <div class="category-logo mx-3"
                        :class="{invisible: props.item?.id_level == 1}"
                        >
                            <Image class="img-fluid of-contain" :src="props.item?.category_logo" width="30" height="30">
                            </Image>
                        </div>
                        <Ionicon :icon_name="props.item?.is_published ? 'eye' : 'eye-off'" class="disabled" font_size="20" />
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between">
                <div class="text-sm-bold fs-14 grey goal-description mx-auto" v-html=" props.item?.description"></div>
                <div
                    ...
                    >
                    <div class="text-sm-bold fs-16 text-uppercase text-warning-2 lh-c-1">{{ eventDate }}</div>
                </div>
            </div>
        </div>
        <div class="card-body card-img p-5"
        :style="[ props.item?.logo != '' ? {'background-image': 'url(' + `https://s3.ap-southeast-1.amazonaws.com/www.thestartupbuddy.co/Next-TSB/User-img/${ encodeURIComponent(props.item?.logo?.trim()) }` + ')'} : { 'background': 'linear-gradient(180deg,rgba(65, 96, 148, 0.15) 75%, #416094 100%)' }]">
        </div>
